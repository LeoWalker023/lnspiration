(() => {
  ...
  const imageView = products.find((product) => product.id === 'imageview');
  if (productsSection && imageView && !productsSection.querySelector('[data-imageview-card]')) {
    productsSection.insertAdjacentHTML('beforeend', `<article class="product-card imageview-card" data-imageview-card><div class="product-copy"><p class="eyebrow">WINDOWS IMAGE TOOL</p><h2>看见每一张<br>图片的细节</h2><p class="product-name">${escape(imageView.name)}</p><p class="description">${escape(imageView.description)}</p><ul>${imageView.features.slice(0,4).map((item) => `<li>${escape(item)}</li>`).join('')}</ul><a class="text-link" href="product.html?product=imageview">进一步了解</a></div><div class="product-visual imageview-visual"><div class="imageview-screen"><div class="imageview-toolbar"><span>Image View</span><i></i><i></i><i></i></div><div class="imageview-canvas"><div class="imageview-photo"><b>IMG</b><small>3840 × 2160</small></div></div><div class="imageview-thumbs"><span></span><span class="active"></span><span></span><span></span><span></span></div></div></div></article>`);
  }
  const desktopVideoPlayer = products.find((product) => product.id === 'desktopvideoplayer');
  if (productsSection && desktopVideoPlayer && !productsSection.querySelector('[data-desktop-video-player-card]')) {
    productsSection.insertAdjacentHTML('beforeend', `<article class="product-card reverse desktop-video-player-card" data-desktop-video-player-card><div class="product-copy"><p class="eyebrow">WINDOWS VIDEO TOOL</p><h2>逐帧查看<br>每个瞬间</h2><p class="product-name">${escape(desktopVideoPlayer.name)}</p><p class="description">${escape(desktopVideoPlayer.description)}</p><ul>${desktopVideoPlayer.features.slice(0,4).map((item) => `<li>${escape(item)}</li>`).join('')}</ul><a class="text-link" href="product.html?product=desktopvideoplayer">进一步了解</a></div><div class="product-visual desktop-video-player-visual"><div class="desktop-video-player-screen"><div class="dvp-toolbar"><span>Desktop Video Player</span><i></i><i></i><i></i></div><div class="dvp-preview"><div class="dvp-scene"><b>FRAME 128</b><small>00:00:04.267</small></div><div class="dvp-side"><span>1920 × 1080</span><span>29.97 fps</span><span>暂停 · 可逐帧</span></div></div><div class="dvp-timeline"><i></i><b></b></div><div class="dvp-controls"><span>◀◀</span><strong>▶</strong><span>▶▶</span><small>1×　🔊</small></div></div></div></article>`);
  }
  ...
  if (displayHeadings[4]) displayHeadings[4].innerHTML = '看见每一张<br>图片的细节';
  if (displayHeadings[5]) displayHeadings[5].innerHTML = '逐帧查看<br>每个瞬间';
  ...
  pluginStyle.textContent = '.plugin-visual{background:linear-gradient(145deg,#3d2b69,#151b3d)}.plugin-screen{width:68%;display:grid;gap:14px;padding:24px;border:1px solid #ffffff22;border-radius:16px;background:#17172aee;box-shadow:0 25px 55px #0008}.plugin-screen span{color:#9694bd;font-size:11px}.plugin-screen strong{font-size:20px}.plugin-screen small{color:#a9a7c7}.plugin-screen button{border:0;border-radius:7px;background:#6755da;color:#fff;padding:9px;font-size:12px}.dependency-visual{background:linear-gradient(145deg,#173d3b,#111d27)}.dependency-screen{width:78%;padding:18px;border:1px solid #ffffff20;border-radius:12px;background:#16191d;box-shadow:0 25px 55px #0008}.dependency-toolbar{display:flex;align-items:center;gap:8px;padding-bottom:13px;border-bottom:1px solid #30363d;font-size:10px;color:#7e8a94}.dependency-toolbar span{margin-right:auto;color:#e4e8eb;font-weight:600}.dependency-toolbar b,.dependency-toolbar i{padding:5px 7px;border-radius:5px;font-style:normal;font-weight:500}.dependency-toolbar b{background:#237b73;color:#e9fffb}.dependency-columns{display:grid;grid-template-columns:1fr 1fr;gap:9px;padding:12px 0}.dependency-columns>div{display:grid;gap:6px;padding:10px;background:#20252a;border-radius:7px}.dependency-columns small{color:#7c8992;font-size:9px}.dependency-columns strong{font-size:11px;color:#f2f4f5}.dependency-columns span{padding:6px;background:#292f35;border-radius:4px;color:#aeb8bf;font-size:9px}.dependency-status{color:#72cfc0;font-size:9px;text-align:right}.imageview-visual{background:linear-gradient(145deg,#3b2b1f,#111923)}.imageview-screen{width:78%;padding:14px;border:1px solid #ffffff20;border-radius:14px;background:#171b21ee;box-shadow:0 25px 55px #0008}.imageview-toolbar{display:flex;align-items:center;gap:6px;color:#9da8b3;font-size:10px;padding:2px 3px 12px}.imageview-toolbar span{margin-right:auto;color:#eef1f4;font-weight:600}.imageview-toolbar i{width:7px;height:7px;border-radius:50%;background:#596572}.imageview-canvas{display:grid;place-items:center;min-height:190px;border-radius:8px;background:repeating-conic-gradient(#252c32 0 25%,#1d2328 0 50%) 50%/18px 18px}.imageview-photo{width:62%;aspect-ratio:1.45;background:linear-gradient(145deg,#e7b36f,#9672d4 48%,#3f678f);border-radius:5px;display:flex;flex-direction:column;align-items:center;justify-content:center;box-shadow:0 12px 25px #0008;color:#fff}.imageview-photo b{font-size:23px;letter-spacing:.08em}.imageview-photo small{margin-top:8px;font-size:9px;color:#ffffffbb}.imageview-thumbs{display:flex;gap:6px;padding-top:11px}.imageview-thumbs span{width:31px;height:22px;border-radius:4px;background:linear-gradient(145deg,#5f6870,#2d343a)}.imageview-thumbs .active{outline:2px solid #d58a4b;background:linear-gradient(145deg,#e7b36f,#9672d4)}.desktop-video-player-visual{background:linear-gradient(145deg,#202f48,#11161e)}.desktop-video-player-screen{width:82%;padding:14px;border:1px solid #ffffff20;border-radius:14px;background:#141a24ee;box-shadow:0 25px 55px #0008}.dvp-toolbar{display:flex;align-items:center;gap:6px;color:#8d9aaa;font-size:10px;padding:2px 3px 12px}.dvp-toolbar span{margin-right:auto;color:#eef3fa;font-weight:600}.dvp-toolbar i{width:7px;height:7px;border-radius:50%;background:#59697b}.dvp-preview{display:grid;grid-template-columns:1.4fr .6fr;gap:8px}.dvp-scene{display:grid;place-items:center;align-content:center;min-height:150px;border-radius:8px;background:linear-gradient(145deg,#556c8e,#17233c 58%,#c08354);box-shadow:inset 0 0 0 1px #ffffff15;color:#f8fbff}.dvp-scene b{font-size:18px}.dvp-scene small{margin-top:8px;color:#c6d2e2;font-size:9px}.dvp-side{display:grid;align-content:center;gap:7px;padding:10px;border-radius:8px;background:#1c2532;color:#aebbd0;font-size:10px}.dvp-side span{padding:7px;border-radius:5px;background:#263244}.dvp-timeline{height:5px;margin:12px 0 9px;border-radius:8px;background:#344255}.dvp-timeline i{display:block;width:54%;height:100%;border-radius:8px;background:#5f9cff}.dvp-timeline b{display:block;width:10px;height:10px;margin-top:-7px;margin-left:53%;border-radius:50%;background:#f3f7ff;box-shadow:0 0 0 3px #5f9cff55}.dvp-controls{display:flex;align-items:center;gap:12px;color:#8493a6;font-size:10px}.dvp-controls strong{color:#f2f6fc;font-size:16px}.dvp-controls small{margin-left:auto;color:#8fa2ba}@media(max-width:720px){.dependency-screen{width:84%}.imageview-screen{width:84%}.desktop-video-player-screen{width:88%}}';
